...
expectType<Proxied<{ x : 42 }>>(extend({ x: 42 }));
expectType<Proxied<[1, 2, 3]>>(extend([1, 2, 3]));
expectType<Proxied<(x : number) => number>>(extend((x : number) => x + 1));

// Check that extension works
expectType<Proxied<null, { ext : true }>>(extend(null, { ext: true }));
expectType<Proxied<{ x : 42 }, { ext : true }>>(extend({ x: 42 }, { ext: true }));
